...
The bridge usermod is intentionally small. It reads one JSON object per line from RS-485, applies state JSON through WLED's JSON state API, applies config JSON through WLED's config API when available, and writes a compact state/info snapshot back to the P4. The snapshot is trimmed to the fields the P4 parser consumes so it stays under the P4 RS-485 receive line limit.
