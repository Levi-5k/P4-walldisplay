...
static void app_config_tuning_clamp(app_tuning_config_t *cfg)
{
    ...
    cfg->power_on_delay_ms = clamp_u16(cfg->power_on_delay_ms, 0, 5000);
...
esp_err_t app_config_tuning_load(app_tuning_config_t *out)
{
    ...
    if (nvs_get_u16(h, "pwr_on_dly", &u16) == ESP_OK) out->power_on_delay_ms = u16;
...
esp_err_t app_config_tuning_save(const app_tuning_config_t *cfg)
{
    ...
    if (err == ESP_OK) err = nvs_set_u16(h, "pwr_on_dly", clean.power_on_delay_ms);
...
esp_err_t app_config_theme_load(app_theme_config_t *out)
{
    ...
    if (nvs_get_u8(h, "slide_en", &u8) == ESP_OK) out->slideshow_enabled = u8 != 0;
...
esp_err_t app_config_theme_save(const app_theme_config_t *cfg)
{
    ...
    if (err == ESP_OK) err = nvs_set_u8(h, "slide_en", cfg->slideshow_enabled ? 1 : 0);
